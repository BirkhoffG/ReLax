...
__all__ = ['NumpyDataset', 'NumpyLoader', 'find_imutable_idx_list', 'DataModuleConfigs', 'TabularDataModule',
           'MNISTDataConfigs', 'FlattenAndCast', 'MNISTDataModule']
...
# Cell
class MNISTDataConfigs(BaseParser):
    batch_size: int = 256
...
def split_x_and_y_MNIST(dataset, labels):
    X = dataset.data
    y = dataset.targets
    data_idx = (y == float(labels[0])) | (y == float(labels[1]))
    data = X[data_idx] / 256.0
    targets = F.one_hot(y)[data_idx][:, int(labels[1])]
...
class MNISTDataModule:
    def __init__(self, configs: Union[Dict[str, Any], MNISTDataConfigs]) -> None:
        self.configs: MNISTDataConfigs = validate_configs(configs, MNISTDataConfigs)
    ...
    def prepare_data(self):
        train_dataset = datasets.MNIST(self.configs.data_dir, train=True, download=True,
                       transform=FlattenAndCast())
        test_dataset = datasets.MNIST(self.configs.data_dir, train=False, download=True,
                       transform=FlattenAndCast())

        train_X, train_y = split_x_and_y_MNIST(train_dataset, self.configs.labels)
        test_X, test_y = split_x_and_y_MNIST(test_dataset, self.configs.labels)
